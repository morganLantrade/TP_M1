...
Fixpoint eval (env : string -> nat) (e : exp) : nat :=
  ...
  | IfThen c e1 e2 => if (eval env c) then (eval env e2) else (eval env e1)
  end.
...
Inductive smallStep : (list nat * asm) -> (list nat * asm) -> Prop :=
  ...
  | ssI2 : forall c stk l1 l2,c <= 0 -> smallStep (c::stk, ifThen l1 l2) (stk, l2)
  | ssI1 : forall c stk l1 l2,c > 0 -> smallStep (c::stk, ifThen l1 l2) (stk, l1).
...
Proof.
  induction e;intros;revert H;simpl.
   -apply bsStep. apply ssPush.
   -apply bsStep. apply ssPush.
   -intro. apply IHe2. apply IHe1. revert H. apply bsStep. apply ssAdd.
   -intro. apply IHe2. apply IHe1. revert H. apply bsStep. apply ssSub.
   -intro. apply IHe1. destruct (eval env e1).
Admitted.
